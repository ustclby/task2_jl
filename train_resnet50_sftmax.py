...
parser.add_argument('--snapshotpath', default='/scratch1/bylu', type=str,
...
def main():
    ...
    viz = visdom.Visdom()
    loss_plt = viz.line(X=np.empty([1]),
                        Y=np.empty([1]),
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, loss_plt)
        ...
        }, args.snapshotpath,args.arch)


def train(train_loader, model, criterion, optimizer, epoch, loss_plt):
    ...
    for i, (input, target) in pbar:
        ...
        pbar.set_postfix({'iter/total':str(i)+'/'+str(len(train_loader)), 'loss':losses.val, 'time/batch':batch_time.val, 'lr':l_r})
...
def save_checkpoint(state, path, arch,  snapshot='snapshot.pth.tar'):
    filename = os.path.join(path, arch + '_' + str(state['epoch']) + '_' + snapshot)
